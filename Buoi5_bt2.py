...
def test_model():
    ...
    # Dự đoán trên tập test cho tất cả các thuật toán đã huấn luyện
    y_predicts = {}
    for algorithm, metrics in error_metrics.items():
        if algorithm == "KNN":
            model = KNeighborsRegressor(n_neighbors=3, p=2)
        elif algorithm == "Linear Regression":
            model = LinearRegression()
        ...
        model.fit(X_train, y_train)
        y_predict = model.predict(X_test)
        y_predicts[algorithm] = y_predict
