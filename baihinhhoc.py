...
# Hàm tính diện tích và thể tích hình lục giác đều
def ve_hinh_luc_giac_3d():
    try:
        ...
        # Vẽ hình lục giác trong không gian 3D
        fig = plt.figure()
        ...
        plt.title(f'Hình lục giác đều với cạnh {a} và chiều cao {h}')
        plt.show()
...
combo_3d = ttk.Combobox(frame_3d, values=["Hình trụ", "Hình cầu", "Hình nón", "Hình lục giác"])
combo_3d.grid(row=0, column=1, padx=10, pady=5)

# Nhập cạnh cho hình lục giác và chiều cao cho các hình trụ, nón, lục giác
label_canh_luc_giac = tk.Label(frame_3d, text="Nhập cạnh hình lục giác:")
